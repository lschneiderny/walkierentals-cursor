...
interface Package {
  ...
  packageItems?: Array<{ rentalId: string; count: number }>;
}
...
export default function ListingManagement() {
  ...
  const [editingItem, setEditingItem] = useState<Rental | Package | RetailItem | null>(null)
  ...
  const handleEdit = (item: Rental | Package | RetailItem) => {
    setEditingItem(item)
    setShowCreateForm(true)
  }
...
function CreateEditForm({
  ...
  type: ListingType
  item?: Rental | Package | RetailItem | null
  categories: Category[]
  onClose: () => void
  onSuccess: () => void
}) {
  ...
  if (item) {
    ...
    initialData.slug = item.slug ?? '';
    initialData.searchTags = item.searchTags ?? '';
    initialData.categoryId = item.category?.id ?? '';

    if (type === 'rentals') {
      const r = item as Rental;
      initialData.dailyRate = r.dailyRate?.toString() ?? '';
      initialData.currentInventory = r.currentInventory?.toString() ?? '';
      initialData.specs = JSON.stringify(r.specs) ?? '';
      initialData.available = r.available ?? true;
    } else if (type === 'packages') {
      const p = item as Package;
      initialData.dailyRate = p.dailyRate?.toString() ?? '';
      initialData.packageItems = JSON.stringify(p.packageItems ?? []) ;
    } else if (type === 'retail') {
      const ri = item as RetailItem;
      initialData.unitCost = ri.unitCost?.toString() ?? '';
      initialData.currentInventory = ri.currentInventory?.toString() ?? '';
      initialData.specs = JSON.stringify(ri.specs) ?? '';
      initialData.available = ri.available ?? true;
    }
  }
